            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        ...
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form action="{{ route('admin.actualites.update', ['actualite' => $actualite]) }}" method="post"
                            ...
                            <div class="card-body" id="inputs">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="categorie_id" id="categorie_id">
                                        ...
                                        <option value="{{ $categorie->id }}" @if($categorie->id ==
                                            $actualite->categorie_id) selected @endif>{{ $categorie->label }}</option>
                                        @endforeach
                                        <option value="<?= null ?>"  @if(($actualite->categorie_label) || ($errors->has('categorie_id')  &&  old('categorie_id') == null)) selected @endif>Autre</option>
                                    </select>
                                    @error('categorie')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                ...
                                @endif
                                @if($actualite->categorie_label)
                                    <div class="form-group categorie_div">
                                        <label for="categorie_label">Catégorie <span style="color: red">* </span> </label>
                                        <input type="text" class="form-control" name="categorie_label" value="{{ $actualite->categorie_label }}" id="categorie_label" placeholder="Saisir categorie">
                                        @error("categorie_label")
                                        <p class="text-danger">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" class="form-control" placeholder="Saisir description"
                                        id="">{{ $actualite->description }}</textarea>
                                    @error('description')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
